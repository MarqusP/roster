...
export function useSettings(uid) {
  const [chapterName, setChapterNameState] = useState("Alpha Kappa Psi — Rho Chapter");

  useEffect(() => {
    if (!firebaseReady || !uid) return undefined;
    const ref = doc(db, "settings", "main");
    const unsubscribe = onSnapshot(ref, (snap) => {
      setChapterNameState(snap.exists() ? snap.data().chapterName || "Alpha Kappa Psi — Rho Chapter" : "Alpha Kappa Psi — Rho Chapter");
    });
    return unsubscribe;
  }, [uid]);
